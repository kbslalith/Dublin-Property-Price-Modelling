...
# round the distances for spatial analysis
...
#####################################################################
#### Check if new changes to postcode have made a positive change #####
...
names(datam)
datam <- datam[c(2:3, 5:26, 200, 209:210)]
names(datam)
...
datay <- subset(datay, select = - c(bedsFact, bathsFact, bedrooms, bathrooms, price_per_m2, north_south, county,
...
new_train1 <- new_train[c(2:3, 5:8, 16:28, 200, 210, 211)]
validation1 <- validation[c(2:3, 5:8, 16:28, 200, 210, 211)]
names(new_train1)
...
## (x) Simple Average RMSE of Random Forest + GBM + XGBoost
rmse(validation2$log_price, (prediction5 + prediction7 + prediction8)/3)
#[1]

## (x) Weighted Average RMSE of RF+GBM+XGBoost
...
## (11) Simple Average RMSE of XGBoost+Ensemble+Meta-Model
...
train11 <- new_train[c(2:3, 5:28, 200, 210, 211)]
test11 <- validation[c(2:3, 5:28, 200, 210, 211)]
names(train11)
...
train1 <- new_train[c(2:3, 5:28, 200, 209, 211)]
test11 <- validation[c(2:3, 5:28, 200, 209, 211)]
names(train1)
...
train31 <- new_train3[c(2:3, 5:28, 200, 210, 211)]
test31 <- validation3[c(2:3, 5:28, 200, 210, 211)]
names(train31)
...
###### Plotting the Predictions vs the asking price ##########
